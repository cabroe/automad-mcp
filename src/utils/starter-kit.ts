...
function getDefaultDescription(filePath: string, type: "file" | "dir"): string | undefined {
  ...
  const fileDescriptions: Record<string, string> = {
    "README.md": "Full setup guide: prerequisites, dev server, theme structure, production build, deployment",
    "theme.json": "Theme metadata: name, description, author, templates, and field definitions for the dashboard",
    ".env.example": "Example environment file – set AUTOMAD_BASE to your Automad installation directory",
    "composer.json": "PHP dependencies (automad/automad) and autoloader config",
    "package.json": "Node.js scripts (dev, build, format) and dependencies (esbuild, LESS, PostCSS, TypeScript)",
    ...
    "pagelist.php": "Pagelist template – displays a filterable/sortable list of child pages with thumbnails",
    "page_not_found.php": "404 error page template",
    "blocks/pagelist/grid.php": "Grid layout template for pagelist block – renders page cards with image, title, excerpt",
    "bin/dev.sh": "Development start script – launches PHP dev server and esbuild watcher in parallel",
