...
export async function analyzeFields(input: FieldAnalyzerInput): Promise<string> {
  ...
  const fieldDefs = existsSync(themeJsonPath) ? JSON.parse(await readFile(themeJsonPath, "utf-8")) : {};
...
function categorizeField(field: string): "page" | "shared" | "system" | "runtime" | "block" {
  ...
  if (field.startsWith("select") || field.startsWith("number") || field.startsWith("date")) return "page";
  return "page";
}
