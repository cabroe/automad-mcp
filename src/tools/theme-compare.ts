...
export const themeCompareInputSchema = z.object({
  baseTheme: z.string().optional().describe('Base theme (usually Starter Kit)'),
  compareTheme: z.string().optional().describe('Theme to compare against base'),
  starterKitAsBase: z
    .boolean()
    .optional()
    .default(true)
...
export async function compareThemes(input: ThemeCompareInput): Promise<string> {
  ...
  if (!compare.hasDist) {
    recommendations.push('Add `dist/` directory and pre-compile CSS/JS for production');
  }

  if (!compare.hasLib) {
    recommendations.push('Add `lib/` directory for helper functions (e.g., `functions.php`)');
  }
...
async function analyzeTheme(themePath: string, _name: string): Promise<ThemeStructure> {
  const files: string[] = [];
