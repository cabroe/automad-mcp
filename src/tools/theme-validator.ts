...
export async function validateTheme(input: ThemeValidatorInput): Promise<string> {
  ...
  // Find theme
  const searchPaths = themePath ? [themePath] : [
  ...
  for (const p of searchPaths) {
    if (existsSync(p)) { themeDir = p; break; }
  }
  ...
  if (existsSync(themeJsonPath)) {
    try {
      ...
      results.push({ name: "theme.json.parse", passed: false, message: "✗ Invalid JSON", severity: "error" });
    }
  }
  ...
  results.push({
    name: "+main block",
    passed: hasMainBlock,
    message: hasMainBlock ? "✓ Uses @{ +main } for block editor" : "⚠️ No @{ +main } found - is block editor supported?",
